 ...
 * @version $Id: ClientController.php 21555 2012-12-11 07:12:32Z gao.wanggao $ 
 * @package 
 *
 */
class ClientController extends AdminBaseController {
	...
	public function beforeAction($handlerAdapter) {
		...
		$config = $ds->getValues('windid');
		if ($config['windid'] == 1) $this->showError('ADMIN:fail');
	}
	...
	public function clientTestAction() {
		...
		if ($client) $this->showError('ADMIN:fail');
		$time = Pw::getTime();
		$params['operation'] = '999';
		$params['params'] =  serialize(array('test'=>$time));
		$windidkey = WindidUtility::appKey($client['id'], $time, $client['secretkey']);
		$url = WindidUtility::buildClientUrl($client['siteurl'], $client['apifile']) . 'windidkey='.$windidkey.'&clentid=' . $client['id'].'&time='.$time;

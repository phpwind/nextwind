...
class PwDebugApplication {
	...
	public function compile() {
		...
		foreach ($manifests as $alias => $manifest) {
			...
			if (isset($logs[$alias]) && md5_file($manifest) == $logs[$alias]) continue;
	...
	private function _read() {
		...
		/* $to_install = array_diff($dirs, $result);
		foreach ($to_install as $pack) {
			if ($pack[0] == '.') continue;
			$this->_toinstall($ext . DIRECTORY_SEPARATOR . $pack);
		} */
		foreach ($result as $k) {
	...
	public function installPack($pack) {
		...
		if (!is_file($manifest)) return $this->_e(null, new PwError('file.not.exist - ' . $manifest));;
	...
	private function _e($install, $r) {
		...
		Wind::getApp()->getResponse()->setBody('<div class="tips">' . $lang->getMessage($error, $var) . '</div>', 'install');
	}
